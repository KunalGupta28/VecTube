...
# Clients to try in order of preference. WEB works best with PO tokens,
# while ANDROID/IOS variants use different InnerTube endpoints that may
# bypass certain connection blocks.
_FALLBACK_CLIENTS = ["WEB", "ANDROID", "ANDROID_VR", "IOS", "WEB_EMBED"]
_MAX_RETRIES = 3
...
class YoutubeService:
    def __init__(self, url):
        self.__agent = self._create_youtube_agent(url)

    @staticmethod
    def _create_youtube_agent(url: str) -> YouTube:
        """Create a YouTube agent with retry logic and client fallback.

        Tries multiple InnerTube clients to work around ConnectionResetError
        and other transient YouTube API issues.
        """
        last_exception = None
        ...
        for client in _FALLBACK_CLIENTS:
            for attempt in range(1, _MAX_RETRIES + 1):
                try:
                    logger.debug(
                        f"Attempting YouTube connection: client={client}, "
                    ...
                    # Force a metadata fetch to verify the connection works
                    _ = agent.title
                    logger.info(
                        f"YouTube connection successful with client={client}"
                    )
                    return agent
                except (
                    ...
                    logger.warning(
                        f"YouTube connection failed (client={client}, "
                        f"attempt={attempt}/{_MAX_RETRIES}): {type(e).__name__}: {e}. "
                        f"Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                except Exception as e:
                    # Non-retryable error (e.g., invalid URL, video unavailable)
                    logger.warning(
                        f"Non-retryable error with client={client}: "
                        f"{type(e).__name__}: {e}. Trying next client..."
                    )
                    last_exception = e
                    break  # Skip retries for this client, try the next

        # All clients and retries exhausted — raise the last captured error
        logger.error(
            f"All YouTube connection attempts exhausted. Last error: {last_exception}"
        )
    ...
    def __extract_chapters(self, required_audio: bool) -> list[VideoChapter]:
        ...
        has_captions = not required_audio
    ...
    def __download_audio(self) -> str:
        ...
        return process_audio(os.path.join(TEMP_AUDIO_DIR, f"{tmp_audio_file_name}.mp4"))
