...
class AiService:
    ...
    def __get_local_embedding_encoder() -> SentenceTransformer:
        if AiService._encoder_cache is not None:
            return AiService._encoder_cache
        ...
        if not os.path.exists(local_model_path):
            ...
            AiService._encoder_cache = encoder
            return encoder
        encoder = SentenceTransformer(
    ...
    def chat_with_gemini(
        ...
        if "1.5-flash" in used_model:
            used_model = "gemini-2.5-flash"
        elif "1.5-pro" in used_model:
            used_model = "gemini-2.5-pro"
        elif "1.0-pro" in used_model:
            used_model = "gemini-2.5-flash"
